...
fintrack account create "Test List Account" --type savings --balance 500.00 2>/dev/null || true
```
...
fintrack account create "Test Show Account" --type checking --balance 2500.50 2>/dev/null || true
```
...
fintrack account create "Original Name" --type checking --balance 1000.00
```
...
fintrack account create "Account to Close" --type savings --balance 100.00
```
...
fintrack account create "JSON Test Account" --type checking --balance 999.99 2>/dev/null || true
```
...
### Execute
```bash
fintrack account create "Invalid Type Account" --type invalid_type --balance 100.00 2>&1
```
...
fintrack account create "Duplicate Name" --type checking --balance 100.00
```

### Execute
```bash
fintrack account create "Duplicate Name" --type savings --balance 200.00 2>&1
```
