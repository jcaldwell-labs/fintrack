...
func (suite *TransactionRepositoryTestSuite) SetupSuite() {
	...
	suite.db = db
	suite.repo = repositories.NewTransactionRepository(db)
	suite.accountRepo = repositories.NewAccountRepository(db)
	suite.categoryRepo = repositories.NewCategoryRepository(db)
